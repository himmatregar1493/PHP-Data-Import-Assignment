...
class ProcessCSVData implements ShouldQueue
{
    ...
    private function insertCommonFeeCollectionHeadwise()
    {
        DB::table('CommonFeeCollection')->orderBy('id')->chunk(1000, function ($collections) {
            ...
            foreach ($collections as $collection) {
                $relatedTransactions = DB::table('Temporary_completedata')->where('receipt_no', $collection->receipt_id)->get();

                foreach ($relatedTransactions as $transaction) {
                    $feeType = DB::table('FeeTypes')->where('f_name', $transaction->fee_head)->first();
    ...
    private function insertFinancialTranDetails()
    {
        DB::table('FinancialTrans')->orderBy('id')->chunk(1000, function ($transactions) {
            ...
            foreach ($transactions as $transaction) {
                $relatedData = DB::table('Temporary_completedata')->where('voucher_no', $transaction->voucher_no)->get();

                foreach ($relatedData as $data) {
                    $feeType = DB::table('FeeTypes')->where('f_name', $data->fee_head)->first();
